                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">{{ __('Attribute Trashbox') }}</h4>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function (xhr) {
                                sessionStorage.setItem('attributeToastMessage', xhr.responseJSON?.message || 'There was an Issue on Restoring the Record.');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            data: { _token: csrfToken },
                            success: function (response) {
                                sessionStorage.setItem('attributeToastMessage', response.message || 'Attribute Deleted Successfully');
                                sessionStorage.setItem('attributeToastType', 'success');
                                window.location.href = '/attribute/trash';
                            },
                            error: function (xhr) {
                                sessionStorage.setItem('attributeToastMessage', xhr.responseJSON?.message || 'There was an Issue on Deleting the Record.');
                                sessionStorage.setItem('attributeToastType', 'failed');
                                window.location.reload();
                            }
                        });
