                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">{{ __('Login History') }}</h4>
                        ...
                        <div class="card-body">
                            <table id="login-history-table" class="table table-hover table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
...
@push('scripts')
    ...
    <script>
        $(document).on('click', '.delete-login-activity', function() {
            const loginActivityID = $(this).data('login-activity-id');
            Swal.fire({
                title: 'Are You Sure?',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '/login-history/' + loginActivityID + '/delete/',
                        method: 'DELETE',
                        data: { "_token": "{{ csrf_token() }}"},
                        success: function (response) {
                            // Swal.fire('Destroyed', 'The Record has been Deleted.', 'success')
                            //     .then(() => { $('#login-history-table').DataTable().ajax.reload(null, false); });

                            $('#login-history-table').DataTable().ajax.reload(null, false);
                            Toastify({
