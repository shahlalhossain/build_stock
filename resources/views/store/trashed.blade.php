                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">{{ __('Store Trashbox') }}</h4>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function (xhr) {
                                sessionStorage.setItem('storeToastMessage', xhr.responseJSON?.message || 'There was an Issue on Restoring the Record.');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            data: { _token: csrfToken },
                            success: function (response) {
                                sessionStorage.setItem('storeToastMessage', response.message || 'Store Deleted Successfully');
                                sessionStorage.setItem('storeToastType', 'success');
                                window.location.href = '/store/trash';
                            },
                            error: function (xhr) {
                                sessionStorage.setItem('storeToastMessage', xhr.responseJSON?.message || 'There was an Issue on Deleting the Record.');
                                sessionStorage.setItem('storeToastType', 'failed');
                                window.location.reload();
                            }
                        });
