    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('is_active', function (Category $category) {
                return $category->is_active ? '<span class="badge bg-success">Yes</span>' : '<span class="badge bg-danger">No</span>';
            })
            ->editColumn('Created By', function (Category $category) {
                return ucwords($category->creator?->name);
            })
            ->editColumn('Updated By', function (Category $category) {
                return ucwords($category->updater?->name);
            })
            ...
            ->addColumn('actions', function (Category $category) {
                if ($this->showTrashed) {
                    return view('category.actions_trashed', ['category' => $category]);
                }

                return view('category.actions', ['category' => $category]);
            })
