...
class UpdateGeoDivisionRequest extends FormRequest
{
    /**
    ...
    {
        $divisionID = $this->route('division')->id ?? null;

        return [
            'name_en' => ['required', 'min:3', 'max:100', 'not_regex:/[0-9]/', Rule::unique('location_divisions', 'name_en')->ignore($divisionID),],
            'name_bn' => ['nullable', 'min:3', 'max:100', 'not_regex:/[0-9]/', Rule::unique('location_divisions', 'name_bn')->ignore($divisionID),],
        ];
    }
